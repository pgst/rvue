class Calender < ApplicationRecord
  ...
  def time_valid
    if self.close_time < self.begin_time
      errors.add(:close_time, "の時刻は、#{self.begin_time.strftime('%H:%M')}より前の時刻にしてください。")
      return
    end
  end
  ...
  def interval_valid
    if self.interval_e < self.interval_s
      errors.add(:interval_e, "の時刻は、#{self.interval_s.strftime('%H:%M')}より前の時刻にしてください。")
      return
    end

    if self.interval_e > self.close_time
      errors.add(:interval_e, "の時刻は、#{self.close_time.strftime('%H:%M')}より前の時刻にしてください。")
      return
    end

    if self.interval_s < self.begin_time
      errors.add(:interval_s, "の時刻は、#{self.begin_time.strftime('%H:%M')}より後の時刻にしてください。")
      return
    end
  end
end
